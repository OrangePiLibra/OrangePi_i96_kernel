...
static int rda_gpio_direction_input(struct gpio_chip *chip, unsigned offset)
{
	...
	printk(KERN_ERR "[%s %d], BIT(%d) %#x\nRegister %#x",
			__func__, __LINE__, offset, (unsigned int)(unsigned long)BIT(offset), 
			(unsigned int)(unsigned long)rda_chip->regs.oen_set_in);
...
rda_gpio_direction_output(struct gpio_chip *chip, unsigned offset, int value)
{
	...
	printk(KERN_ERR "[%s %d], BIT(%d) %#x\nRegister SET %#x\nCLR %#x Value %d\n",
			__func__, __LINE__, offset, (unsigned int)(unsigned long)BIT(offset), 
			(unsigned int)(unsigned long)rda_chip->regs.set, 
			(unsigned int)(unsigned long)rda_chip->regs.clr, value);
